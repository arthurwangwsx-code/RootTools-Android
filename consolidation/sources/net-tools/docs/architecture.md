# Net Tools Architecture

## 1. Scope

Net Tools is a root-first Android inspection application with two deliberately independent observability products:
...
```mermaid
flowchart TB
    subgraph AndroidApp[Net Tools APK]
...
Embedding Python and mitmproxy into the main APK would increase size, startup cost, and failure blast radius. Net Tools therefore communicates with the official PCAPdroid MITM add-on through its Messenger contract.
...
The built-in MITM flow trusts the CA generated by the add-on itself. A separately generated Net Tools CA is retained only for future/custom proxy backends.

Certificate state is represented as three distinct facts:

- certificate available in Net Tools
...
The MITM add-on sends a framing header followed by payload bytes. Net Tools maps these to `DecryptedEvent` values such as:
...
`references/` is intentionally ignored and is not part of the Net Tools source history.
